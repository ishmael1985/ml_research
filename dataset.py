...
class DatasetFromFolder:
    ...
    def _make_subimages(self):
        scale = self.params["upscale_factor"]
        block_size = self.params["input_size"]
        ...
        for x in range(0, h - block_size + 1, stride):
            for y in range(0, w - block_size + 1, stride):
                ...
                # Reshape sub-inputs
                sub_input = sub_input.reshape([block_size, block_size, channels])
        ...
        for x in range(0, h * scale - block_size * scale + 1, stride * scale):
            for y in range(0, w * scale - block_size * scale + 1, stride * scale):
                ...
                # Reshape sub-label
                sub_label = sub_label.reshape([block_size * scale , block_size * scale, channels])
