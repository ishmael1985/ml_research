...
class DatasetFromFolder:
    ...
    def __enter__(self):
        if self.hdf5_path:
            ...
            input_size = self.params["input_size"][scale-2]
            label_size = self.params["label_size"][scale-2]
            
            with h5py.File(self.hdf5_path, 'w') as hf:
                hf.create_dataset('data', (0, input_size, input_size,
                                           self.params["channels"]),
                                  maxshape=(None, input_size, input_size,
                                            self.params["channels"]),
                                  dtype='f4', compression=9,
                                  chunks=(128, input_size, input_size,
                                          self.params["channels"]))
                hf.create_dataset('label', (0, label_size, label_size,
                                            self.params["channels"]),
                                  maxshape=(None, label_size, label_size,
                                            self.params["channels"]),
                                  dtype='f4', compression=9,
                                  chunks=(128, label_size, label_size,
                                          self.params["channels"]))
        return self
    ...
    def _make_subimages(self):
        scale = self.params["upscale_factor"]
        patch_size = self.params["input_size"][scale - 2]
        label_size = self.params["label_size"][scale - 2]
        ...
        padding = int(abs(patch_size - label_size) / 2) # eg. for 3x: (21 - 11) / 2 = 5
        label_padding = int(label_size / scale) # eg. for 3x: 21 / 3 = 7

        for x in range(0, h - patch_size - padding + 1, stride):
            for y in range(0, w - patch_size - padding + 1, stride):
                x_loc = x + label_padding
                y_loc = y + label_padding
                
                sub_input = input_[x + padding : x + padding + patch_size, y + padding : y + padding + patch_size]
                ...
                sub_input = sub_input.reshape([patch_size, patch_size, channels])
                sub_label = sub_label.reshape([label_size, label_size, channels])
